...
export const getCompaniesWithStock = async (productId = "") => {
  ...
  let data = null;
  try {
    const response = await fetch("https://newccacnodemongodb-production.up.railway.app/api/v1/companyList");
...
export const getProductById = async (productId = "") => {
  ...
  try {
    const response = await fetch(`https://newccacnodemongodb-production.up.railway.app/api/v1/products/${productId}`);
    const data = await response.json();
